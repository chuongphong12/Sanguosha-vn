...
export const gangLieSkill: SkillDefinition = {
  id: "gang-lie",
  triggerOn: "AfterDamage",
  canInvoke: (G, playerID, context) => context.targetID === playerID,
  onTrigger: (G, baseEffect, shuffle) => {
    const effect = baseEffect as SkillTriggerEffect;
    const owner = effect.owner;
    const sourceID = effect.context.effect.sourceID;
    if (!G.players[owner].alive || !sourceID || !G.players[sourceID]?.alive) {
      G.effectStack.shift();
      return;
    }
    if (!effect.state) {
  ...
  onAnswer: (G, baseEffect, answer, shuffle) => {
    const effect = baseEffect as SkillTriggerEffect;
    if (G.prompt?.reason === "gang-lie") {
      if (answer.choice === "activate") {
        effect.state = "activated";
        writeLog(G, playerName(G, effect.owner) + " dùng 【Cương Liệt】.");
      } else {
        effect.state = "declined";
      }
      return true;
    }
    if (G.prompt?.reason === "gui-cai") {
      if (answer.choice === "activate") {
        G.prompt = {
          id: G.nextResolutionID++,
          effectID: effect.id,
          kind: "select-cards",
          responderID: G.prompt.responderID,
          reason: "gui-cai",
    ...
    if (G.prompt?.kind === "select-cards" && G.prompt.reason === "gui-cai") {
      const cardIDs = moveSelectedCard(G, G.prompt, answer);
      if (!cardIDs) return false;
      const cardID = cardIDs[0];
      removeZoneCard(G, G.prompt.ownerID, cardID);
      G.discard.push(cardID);
      effect.judgeCardID = cardID;
      effect.state = "gui-cai-changed";
      const judgeCard = G.cards[cardID];
      writeLog(
        G,
        playerName(G, G.prompt.ownerID) +
          " dùng 【Quỷ Tài】 đổi phán xét thành " +
          judgeCard.suit +
          " " +
          judgeCard.rank +
          ".",
    ...
    if (G.prompt?.reason === "gang-lie-punish") {
      const sourceID = effect.context.effect.sourceID;
      if (answer.choice === "discard-2") {
        G.prompt = {
          id: G.nextResolutionID++,
          effectID: effect.id,
          kind: "select-cards",
    ...
    if (G.prompt?.reason === "gang-lie-discard") {
      const cardIDs = moveSelectedCard(G, G.prompt, answer);
      if (!cardIDs) return false;
      const sourceID = effect.context.effect.sourceID;
      for (const cardID of cardIDs) {
